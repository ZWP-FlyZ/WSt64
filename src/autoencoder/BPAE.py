...
#         tmp = np.matmul(gjs,origin_w2.T);
    ...
    def train(self,X,extX,learn_param,repeat,save_path=None,mask_value=0,weight_list=None):
        '''
        注意输入X为一个矩阵(batch,x_size)
        extX为假设原数据集
        '''
...
#                 x = X[start:end,:];
                ...
                mae,rmse=self.evel(py, extx,mask_value);
                err_weight=1.0;
                if weight_list is not None:
                    err_weight = weight_list[i];
                self.layer_optimize(py,extx,x,learn_rate=lr,err_weight=err_weight);
                maeAll+=mae/shape1;
                rmseAll+=rmse/shape1;
#             print(py);
            if rep>0 and rep%de_repeat==0:
